...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # profile.profile_picture = 'default/avatar2.png'
            # print("Image set")
            profile.save()
            # print("Profile saved")

            # USER ACTIVITION AND EMAIL VERIFICATION
        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # getting common variatiton is product variation and ex var list
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        user.save()
        messages.success(
            request, "Congratulations! Your account has been activated successfully.Login Now.")
        return redirect('login')
    else:
...
def my_orders(request):
    # The -created_at : the hyphens created at will display the order descending order
